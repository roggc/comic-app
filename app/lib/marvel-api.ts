...
// lib/api.ts
export async function getMarvelData(type: string, nameStartsWith?: string) {
    ...
    const res = await fetch(
        `${marvelBaseUrl}/${type}?ts=${ts}&apikey=${publicKey}&hash=${hash}&limit=50${nameStartsWith && nameStartsWith !== '' ? `&nameStartsWith=${nameStartsWith}` : ''}`,
        {
